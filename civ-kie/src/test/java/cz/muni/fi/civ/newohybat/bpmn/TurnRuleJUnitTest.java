...
public class TurnRuleJUnitTest extends BaseJUnitTest {
	...
	@Test
    public void testTileIrrigateInTurnRegime(){
    	...
    	settler.getActions().add("move");
    	settler.getActions().add("irrigation");
    	
    	TileDTO tile1 = getTile(1L,2L,2L, "plains",null);
		TileDTO tile2 = getTile(2L,2L,3L, "plains",null);
		tile2.getImprovements().add("irrigation");
		TileDTO tile3 = getTile(3L,3L,3L, "swamp","oil");
		TileDTO tile4 = getTile(4L,3L,2L, "hills","coal");
		
		
    	UnitDTO unit = getUnit(1L, "settler", player1.getId());
    	unit.setTile(tile1.getId());
    	
    	TileImprovementDTO imp = new TileImprovementDTO();
    	imp.setIdent("irrigation");
    	imp.setCost(1);	// it takes one turn to finish
		...
		ksession.getEntryPoint("GameControlStream").insert(new TurnEvent());
		((StatefulKnowledgeSessionImpl)ksession).session.getAgenda().activateRuleFlowGroup("processTiles");
		ksession.fireAllRules();
		...
		Assert.assertTrue("Unit can build irrigation",unit.getActions().contains("irrigation"));

		unit.setCurrentAction("irrigation");
		...
		Assert.assertTrue("Tile was irrigated", tile1.getImprovements().contains("irrigation"));
		...
		Assert.assertTrue("Tile irrigation effect active", tile1.getFoodProduction()==2);
		Assert.assertTrue("unit can irrigate another one", unit.getActions().contains("irrigation"));
    }
    @Test
